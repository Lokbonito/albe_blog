...
class Slides extends Component
{
    ...
    public function createSlide()
    {
        ...
        $this->validate([
            ...
            'slide_heading.required' => 'Vui lòng nhập tiêu đề của slide.',
            'slide_link.url' => 'Đường dẫn slide không hợp lệ. Vui lòng nhập đúng định dạng URL.',
            'slide_image.required' => 'Vui lòng chọn hình ảnh cho slide.',
            'slide_image.mimes' => 'Hình ảnh phải có định dạng PNG, JPG hoặc JPEG.',
            'slide_image.max' => 'Kích thước hình ảnh không được vượt quá 2MB.',
        ]);
        ...
        if (!$upload) {
            $this->dispatch('showToastr', [
                'type' => 'error',
                'message' => 'Có lỗi xảy ra khi tải lên ảnh slide.'
            ]);
        } else {
            /** Store new slide */
            $slide = new Slide();
            ...
            if ($saved) {
                ...
                $this->dispatch('showToastr', [
                    'type' => 'success',
                    'message' => 'Ảnh slide đã được cập nhật thành công'
                ]);
            } else {
                $this->dispatch('showToastr', [
                    'type' => 'error',
                    'message' => 'Có lỗi xảy ra khi lưu ảnh.'
                ]);
            }
        }
    }
    ...
    public function updateSlide()
    {
        ...
        if ($saved) {
            $this->hideSlideModalForm();
            $this->dispatch('showToastr', ['type' => 'success', 'message' => 'Ảnh slide đã được cập nhật thành công']);
        } else {
            $this->dispatch('showToastr', ['type' => 'error', 'message' => 'Có lỗi xảy ra khi cập nhật ảnh.']);
        }
    }
    ...
    public function updateSlidesOrdering($positions)
    {
        ...
        $this->dispatch("showToastr", ["type" => "success", "message" => "Thứ tự slide đã được cập nhật thành công"]);
    }
    ...
    public function deleteSlideAction($id)
    {
        ...
        if ($delete) {
            $this->dispatch('showToastr', ['type' => 'success', 'message' => 'Ảnh slide đã được xóa thành công']);
        } else {
            $this->dispatch('showToastr', ['type' => 'error', 'message' => 'Đã xóa thành công slide.']);
        }
    }
